...
infile <- 'fish_trip_2007_0Oct.csv' #5010 observations - 1.4 hours
...
write.table(alldata, file=paste0("C:/Sources/datafiles/fishChlor_",substr(infile,start=17,stop=19),"07.csv"),sep=",",row.names=F)
...
# Write the Data to CSV File
write.table(alldata, file=paste0("C:/Sources/datafiles/fishDepth_",substr(infile,start=17,stop=19),"07.csv"),sep=",",row.names=F)
...
z + geom_polygon(data = w, aes(x=long, y = lat, group = group), fill = "grey80") +
  ...
  coord_fixed(1.3,xlim = xlim, ylim = ylim) + ggtitle("Mean chl-a Values at fish Landings, Oct, 2007")
...
z + geom_polygon(data = w, aes(x=long, y = lat, group = group), fill = "grey80") +
  ...
  coord_fixed(1.3,xlim = xlim, ylim = ylim) + ggtitle("Bathymetry at fish Landings, July, 2007")
